...
def check_alternate_resname(resname):
    if resname in g_var.alt_res_name:
        ...
        raise ValueError('The residue '+resname+' cannot be found in the topology or alternate resnames')
    else:
        return resname
...
def fetch_bond_info(residue, rtp, at_mass, location):
    ...
    if 'atoms' not in locals():
        print('Issue finding information for residue: ',residue)
        # raise ValueError('There is a issue with: \n'+rtp_file)
    if not residue_present:
        print('cannot find topology information for: '+residue)
        # raise ValueError('cannot find topology information for: '+residue)
...
#### runs through dirctories looking for the atomistic fragments returns the correct location
    ...
    raise ValueError('Cannot find fragment: '+residue+'/'+swap_to_solvent(residue)+'.pdb')
...
def fetch_RNA_mapping():
    mapping_dir = g_var.database_dir + 'Mapping/'
    mapping_file = [f for f in os.listdir(mapping_dir) if f.endswith(f'{g_var.RNA_ff}.map')]
    for file in mapping_file:
        g_var.rna_residues += file.split('.')[0].upper()
