...
#### pdb2gmx on on protein chain, creates the topologies    
    ...
    if res_type == 'RNA' or res_type == 'OTHER':
        ...
        -p '+res_type+'_'+input+str(chain)+'.top  -i '+res_type+'_'+str(chain)+'_posre.itp -ter '+pdb2gmx_selections+'\nEOF', ''+res_type+'_'+input+str(chain)+'_gmx.pdb']) #### single chains
    else:
        gromacs([g_var.args.gmx+' pdb2gmx -f '+res_type+'_'+input+str(chain)+'.pdb -o '+res_type+'_'+input+str(chain)+'_gmx.pdb -water none \
...
def minimise_protein_chain(chain, input, res_type):    
    # "gmx_mpi editconf -f $GRO -o $BW_boxed -c -d 1.0 -bt cubic
    # gromacs([g_var.args.gmx+' editconf -f '+res_type+'_'+input+str(chain)+'_gmx_checked.pdb -o '+res_type+'_'+input+str(chain)+'_gmx_checked_box.pdb -d 1.0 -bt cubic', res_type+'_'+input+str(chain)+'_gmx_checked_box.pdb'])
    #### grompps each protein chain
    # gromacs([g_var.args.gmx+' grompp '+
                # '-f em_'+res_type+'.mdp '+
                # '-p topol_'+res_type+'_'+input+str(chain)+'.top '+
                # '-c '+res_type+'_'+input+str(chain)+'_gmx_checked_box.pdb '+
...
# def minimise_rna_chain(chain, input, res_type):
#     gromacs([g_var.args.gmx+' editconf -f '+res_type+'_'+input+str(chain)+'.gro -o '+res_type+'_'+input+str(chain)+'_box.gro -c -d 1.0 -bt cubic', res_type+'_'+input+str(chain)+'_box.gro'])
#     gromacs([g_var.args.gmx+' grompp '+
#                 '-f em_'+res_type+'.mdp '+
#                 '-p topol_'+res_type+'_'+input+str(chain)+'.top '+
#                 '-c '+res_type+'_'+input+str(chain)+'_box.gro '+
#                 '-o MIN/'+res_type+'_'+input+str(chain)+' '+
#                 '-maxwarn 1 ', 'MIN/'+res_type+'_'+input+str(chain)+'.tpr'])
#     os.chdir('MIN')
#     gromacs([g_var.args.gmx+' mdrun -v -deffnm '+res_type+'_'+input+str(chain)+' -c '+res_type+'_'+input+str(chain)+'.pdb', res_type+'_'+input+str(chain)+'.pdb'])
...
#### open topology file
    ...
    #### individual number of residues
        if residue_type.split('_')[0] in ['PROTEIN', 'OTHER', 'RNA']:
...
def write_merged_topol():
    ...
    #### runs through residue types and copies to MERGED directory and simplifies the names
        ...
        #### copies across protein itp files and simplifies the names 
            if residue_type in ['PROTEIN', 'OTHER', 'RNA']:
...
#### grompps final merged systems
    # gromacs([g_var.args.gmx+' editconf -f merged_cg2at'+protein+'.pdb -o merged_cg2at'+protein+'_box.pdb -d 1.0 -bt cubic', 'merged_cg2at'+protein+'_box.pdb'])
    # gromacs([g_var.args.gmx+' grompp '+
    #         '-po md_out-merged_cg2at '+
    #         '-f em_merged_cg2at.mdp '+
    #         '-p topol_final.top '+
    #         '-r merged_cg2at'+protein+'_box.pdb '+
    #         '-c merged_cg2at'+protein+'_box.pdb '+
