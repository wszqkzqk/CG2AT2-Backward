...
import gen, gro, at_mod, at_mod_p, at_mod_np, read_in, g_var, check_library, at_mod_rna
...
# Nothing in the script below here should need changing by the user
    ...
    if 'PROTEIN' in g_var.cg_residues:          
        ...
        if g_var.user_at_input:
            ...
            read_in.duplicate_chain()  ## duplicates user chains
    ...
    if len([key for value, key in enumerate(g_var.cg_residues) if key not in ['PROTEIN', 'RNA', 'OTHER']]) > 0:
