...
#         if os.path.exists(file+'_'+str(chain)+end):
#             with open(file+'_'+str(chain)+end, 'r') as pdb_input:
#                 temp = read_in.filter_input(pdb_input.readlines(), False)
#                 # merge_temp += read_in.filter_input(pdb_input.readlines(), False)
#         else:
#             sys.exit('cannot find chain: '+file+'_'+str(chain)+end)
#         # translate the atoms to the previous box
#         with open(f'{g_var.working_dir}{res_type}/{res_type}_de_novo_{chain}_gmx_checked.pdb') as pdb_input:
#             pre = read_in.filter_input(pdb_input.readlines(), False)
#             merge_temp += align_atoms(pre, temp)
#         if res_type+'_aligned' in file:  
#             count, restraint_count = at_mod_p.create_disres(merge_temp, chain, file, count, restraint_count)
#         merge, merge_coords = fix_chirality(merge,merge_temp,merged_coords, res_type)   
#     if res_type+'_aligned' not in file:
...
def align_atoms(residues1, residues2):
    positions1 = np.array([[residue['x'], residue['y'], residue['z']] for residue in residues1])
    positions2 = np.array([[residue['x'], residue['y'], residue['z']] for residue in residues2])
    align_ = _get_rmsd_loss(positions1, positions2)
    # rot = align_['rot']
    # trans = align_['trans']
    align_fn = align_['align']
    position1_new = align_fn(positions2)
    for i, residue in enumerate(residues2):
        residue['x'], residue['y'], residue['z'] = float(position1_new[i][0]), float(position1_new[i][1]), float(position1_new[i][2])
    return residues2
...
#   return {"rmsd":rmsd, "align":align_fn, "rot":aln, "trans": (T_mu-P_mu @ aln)[:, 0, :]}

def _kabsch(a, b, return_v=False):
    # '''get alignment matrix for two sets of coordinates using PyTorch'''
    # with torch.cuda.amp.autocast(enabled=False):
    ab = np.matmul(a.transpose(-1, -2), b)
    u, s, vh = np.linalg.svd(ab, full_matrices=False)
    flip = np.linalg.det(np.matmul(u, vh)) < 0
    u_ = np.where(flip, -u[..., -1], u[..., -1]).reshape(-1,1) 
    u = np.concatenate((u[..., :-1], u_), axis=-1) if flip.any() else u 
    return u if return_v else np.matmul(u, vh)
    ...
    #### run through every residue type in cg_residues
        ...
        for segment, residue_type in enumerate(g_var.system):
            if residue_type not in done:
                if residue_type not in ['PROTEIN', 'OTHER', 'RNA']:
...
def fetch_chiral_coord(merge_temp, residue_type):
    ...
    for atom in range(len(merge_temp)):
        if residue_type == 'RNA':
            ...
            continue
        elif residue_type in ['PROTEIN', 'OTHER']:
