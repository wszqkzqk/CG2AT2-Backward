...
#### updates dictionaries and counters
    ...
    for key in g_var.cg_residues:
        ...
        for residue in g_var.cg_residues[key]:
            chain_id = g_var.cg_residues[key][residue][next(iter(g_var.cg_residues[key][residue]))]['chain_id']
            if chain_id not in g_var.cg_chain_id[key]:
                g_var.cg_chain_id[key].append(chain_id)
    return box_vec[0]
...
def add_to_cg_database(line_sep_prev, count, residue_list):
    ...
    elif line_sep_prev['residue_name'] in g_var.rna_residues:
        g_var.cg_residues['RNA'][count]={} ### then create sub dictionary cg_residues['PROTEIN'][count]
        g_var.cg_residues['RNA'][count]=residue_list ### adds residue list to dictionary key cg_residues['PROTEIN'][count]
    else:
...
def add_residue_to_dictionary(line_sep):
    if line_sep['residue_name'] in g_var.p_residues: ## if in protein database 
        ...
        pass
    elif line_sep['residue_name'] in g_var.rna_residues:
        if "RNA" not in g_var.cg_residues:
            g_var.cg_residues["RNA"]={}
    else:
